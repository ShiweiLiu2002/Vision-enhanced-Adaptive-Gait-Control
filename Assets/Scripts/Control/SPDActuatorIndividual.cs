...
namespace ModularAgents.MotorControl.Mujoco
{
    ...
    /// </summary>
    public class SPDActuatorIndividual : ActuatorComponent
    {
        [SerializeField]
        ...
        public unsafe void MjInitialize()
        {
            ...
            // 初始化每个 DOF 的 KP、KD 数组
            double[] kpArray = new double[dofAddresses.Length];
            double[] kdArray = new double[dofAddresses.Length];

            int index = 0;
            foreach (var js in jointStates)
            {
                double kp, kd;
                ...
                {
                    var gain = jointPDGains.FirstOrDefault(g => js.Joint.name.Contains(g.jointName));
                    kp = gain?.kp ?? 200.0;
                    kd = gain?.kd ?? 5.0;
                }

                int dof = js.Joint.DofCount();
                for (int i = 0; i < dof; i++)
                {
                    kpArray[index] = kp;
                    kdArray[index] = kd;
...
posGainMatrix = Matrix<double>.Build.DiagonalOfDiagonalVector(Vector<double>.Build.DenseOfArray(kpArray));
velGainMatrix = Matrix<double>.Build.DiagonalOfDiagonalVector(Vector<double>.Build.DenseOfArray(kdArray));
        ...
        {
            SPDActuatorIndividual component;

            public SPDActuator(SPDActuatorIndividual component)
            {
